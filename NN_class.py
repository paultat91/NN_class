...
class NN(object):
    """
    A one hidden layer Neural Network similar to the MLPClassifier in the scipy library
    """
    def __init__(self, hidden_layer_size=1, activation='sigmoid', learning_rate=0.1, max_iter=1000, bias=True):
        self.learning_rate = learning_rate
        self.lessons = max_iter
        self.act = activation   
    ...
    def activation(self, x):
        if self.act == 'sigmoid' or self.act == 'logistic' :
            return 1/(np.exp(-x) + 1) 
        if self.act == 'tanh':
            return np.tanh(x)
        if self.act == 'relu':
