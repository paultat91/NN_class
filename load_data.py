...
# MY NEURAL NETWORK
Neural = NN(hidden_layer_size=1, activation='logistic', learning_rate=.1, max_iter=1000, bias=False)
acc = Neural.train_test(x, y)
print("Score on test set from my NN:", acc)
MY_weights = Neural.final_weights

# SCIPY MLPClassifier
from sklearn.neural_network import MLPClassifier
from sklearn.model_selection import train_test_split
X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=56)
model = MLPClassifier(hidden_layer_sizes=(1,1), activation='logistic', learning_rate_init=.1, max_iter=1000, solver='lbfgs')
model.fit(X_train, y_train);
score = model.score(X_test, y_test)
print("Score on test set from MLP:", score)
MLP_weights = [model.coefs_, model.intercepts_]
